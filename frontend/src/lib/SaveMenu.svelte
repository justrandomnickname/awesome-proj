<script lang="ts">
	...
	async function loadGame(filename: string) {
		try {
			loading = true
			console.log("Starting to load game:", filename)
			await LoadGame(filename)
			console.log("Game loaded successfully, dispatching event")
			closeMenu()
							...
							{#each saves as save}
								<div class="save-item">
									...
									<div class="save-actions">
										<button
											class="load-btn"
											on:click={() => loadGame(save.filename)}
											disabled={loading}>
											📂 Загрузить
										</button>
										<button
											class="delete-btn"
											on:click={() => deleteSave(save.filename, save.name)}
											disabled={loading}>
											🗑️ Удалить
										</button>
									</div>
